...
from utils.PoseEstimation import arun, teaser_solver, estimate_pose_from_2d2d
from utils import Clusters
from utils.limo_rejection import *
...
iparams.sigma = 0.4             # decay speed
iparams.epsilon = 0.6           # distance cutoff
...
for path_idx in range(START_IDX, len(path_quer_all)):
    ...
    for cam in cameras:
        ...
        for k_idx in range(nn_info.num_kps):
            ...
            for bin_idx in range(min(SIGNIF_BINS, len(signif_bins))): # choose closest 2 signif bins:
                ...
                # No points in bin, skip getting centroid and saving
                if pts_keep.shape[0] == 0:
                    ...
                    continue
                # -----------------------
                # Get lidar points in bin for plane fitting
                # -----------------------
                bin_pts_temp.append(pts_keep)
                print("Adding pts to bin", pts_keep.shape)
            ...
            bins_pts_pkppb.append(bin_pts_temp)
        ...
        bins_pts_pkppb_allcams[cam.name] = bins_pts_pkppb
    # ------------------------
    ...
    if PLOT_FLAG:
        ...
        # # if len(inlier_pts_cam2) > MIN_NUM_INLIERS and abs(trans_err_arun[0]) < 3  and abs(trans_err_arun[1]) < 3:           
            ...
            # H_rq_gt = H_lidar_cam3 @ H_imu_lidar @ np.linalg.inv(H_quer_global_true) @ \
            #             H_retr_global_true @ np.linalg.inv(H_imu_lidar) @ np.linalg.inv(H_lidar_cam2)
            H_rq_gt = np.linalg.inv(H_quer_global_true) @ \
                        H_retr_global_true
            # H_rq_gt = H_lidar_cam2 @ H_imu_lidar @ np.linalg.inv(H_retr_global_true) @ \
            #             H_retr_global_true @ np.linalg.inv(H_imu_lidar) @ np.linalg.inv(H_lidar_cam2)
            # print("H")
            # print(H_rq_gt)
            # print("\nArun translational error (m):", trans_err_arun)
            ...
            viz.show_points_3d_same_plot_query(cameras, pc_clustered_clipper, H_rq_arun, centroid_size=25, title=" Predicted Alignment in Query Cam Frame")
            ...
            # viz.show_points_3d_same_plot_global(cameras, pc_clustered_clipper, [H_retr_global_true, H_quer_global_true], H_imu_lidar, centroid_size=25, title=" Ground Truth Alignment in World Frame")
            
            plot_all_plane_fits(cameras[0].pts_infront, bins_pts_pkppb_allcams["cam2"])
            plot_all_plane_fits(cameras[1].pts_infront, bins_pts_pkppb_allcams["cam3"])
            plt.show()
