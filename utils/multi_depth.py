...
from clipper_temp import cluster_info_to_clipper_list, fuse_matrix_geometric, fuse_matrix_weighted, build_C_from_M
...
from PoseEstimation import arun, teaser_solver, estimate_pose_from_2d2d
import Clusters
...
# For CLIPPER
SIGMA = 0.4         # decay speed
EPSILON = 0.6       # distance cutoff
...
for path_idx in range(START_IDX, len(path_quer_all)):
    ...
    for cam in cameras:
        ...
        print("\nNum keypoints total", len(nn_info.keypoints), nn_info.num_kps, len(nn_info.pxs_per_kp))
    ...
    pts_D1 = pc_clustered_filtered["cam2"].pts.astype(np.float64)
    pts_D2 = pc_clustered_filtered["cam3"].pts.astype(np.float64)
    ...
    descs_D1 = np.array(descs_D1)
    descs_D2 = np.array(descs_D2)
    scores_D = np.array(scores_D)

    # A_init = clipperpy.utils.create_all_to_all(len(pts_D1), len(pts_D2))
    ...
    # Distance + descriptor
    # D1 = np.array([cluster_info_to_clipper_list(pt, desc) for pt, desc in zip(pts_D1, descs_D1)]).T
    # D2 = np.array([cluster_info_to_clipper_list(pt, desc) for pt, desc in zip(pts_D2, descs_D2)]).T
    ...
    if D1.shape[0] == 3:
        ...
        A_init = np.column_stack((idx, idx))
        ...
        iparams.point_dim = 3
        # iparams.method = 'roman'
        iparams.fusion_method = clipperpy.invariants.ROMAN.GEOMETRIC_MEAN
        iparams.ratio_feature_dim = 0   # using pca, volume, etc
        iparams.cos_feature_dim = 0   # size of feature descriptors. LightGlue is 256
        iparams.sigma = SIGMA
        iparams.epsilon = EPSILON
        # iparams.drift_aware = True
        # iparams.cosine_weight = 0.5

        # print("drift_aware",iparams.drift_aware)
        # print("drift_scale_sigma",iparams.drift_scale_sigma)
        # print("drift_scale",iparams.drift_scale)
        ...
        clipper.score_pairwise_and_single_consistency(D1, D2, A_init)
        ...
        for idx1, idx2 in zip(idxs1, idxs2):
            inlier_kis_cam2.append(pc_clustered_filtered["cam2"].kidxs[idx1])
            inlier_kis_cam3.append(pc_clustered_filtered["cam3"].kidxs[idx2])
            ...
            inlier_m_kpxs_cam2.append(m_kpxs_cam2[pc_clustered_filtered["cam2"].kidxs[idx1]])
            inlier_m_kpxs_cam3.append(m_kpxs_cam3[pc_clustered_filtered["cam3"].kidxs[idx2]])
        ...
        pc_clustered_clipper["cam2"] = PointCloudClustered(
                                                        kidxs=np.array(inlier_kis_cam2),
        ...
        # # Testing relative pose est no clipper
        # pc_clustered_clipper["cam2"] = pc_clustered_filtered["cam2"]
        # pc_clustered_clipper["cam3"] = pc_clustered_filtered["cam3"]
        # inlier_pts_cam2 = pc_clustered_filtered["cam2"].pts
        # inlier_pts_cam3 = pc_clustered_filtered["cam3"].pts
        # inlier_pxs_cam3 = pc_clustered_filtered["cam3"].pxs


        # print("\n")
        # print(inlier_kis_cam2)
        # print(inlier_kis_cam3)
        ...
        if len(inlier_pts_cam2) > MIN_NUM_INLIERS:
            ...
            H_quer_global_pred_pnp = H_retr_global_true @ invert_H(H_rq_pnp)


            # ============================================================
            # Essential matrix : relative pose (retr → quer)
            # ============================================================
            R_rq_ess, t_rq_ess, _ = estimate_pose_from_2d2d(inlier_m_kpxs_cam2, inlier_m_kpxs_cam3, cameras[0].K, cameras[0].dist, cameras[1].K, cameras[1].dist)
            H_rq_ess = build_homogeneous(R_rq_ess, t_rq_ess)
            H_quer_global_pred_ess = invert_H(H_retr_global_true) @ invert_H(H_rq_ess)
            ...
            t_ess  = H_quer_global_pred_ess[:3, 3]

            trans_err_arun = t_true - t_arun
            trans_err_pnp  = t_true - t_pnp 
            trans_err_ess  = t_true - t_ess
        ...
        data.points = o3d.utility.Vector3dVector(pts_D2)
        ...
        #     viz.show_lidar_2d(cameras) # all lidar points
        #     viz.show_lidar_neighbors_2d(cameras, keypoints, nn)
        #     viz.show_points_2d(cameras, keypoints, pc_clustered_filtered, title=" Matched Centroids")
        #     viz.show_points_2d(cameras, keypoints, pc_clustered_clipper, kp_size=35, title=" Centroids CLIPPER")
        #     viz.show_points_3d(cameras, pc_clustered_filtered, centroid_size=25, title=" Matched Centroids")
        #     viz.show_points_3d(cameras, pc_clustered_clipper, centroid_size=25, title=" Centroids After CLIPPER")
        ...
        #     #     # Cluster centroids
        #     #     # ax.scatter(
        #     #     #     inlier_m_kpxs[cam.name][:, 0],
        #     #     #     inlier_m_kpxs[cam.name][:, 1],
        #     #     #     s=35,
        #     #     #     # c="cyan",
        #     #     #     c=pc_clustered_clipper[cam.name].kidxs,
        #     #     #     cmap="turbo",
        ...
        #     #         s=35,
        #     #         # c="cyan",
        #     #         c=np.arange(keypoints[cam.name].shape[0]),
        #     #         cmap="turbo",
        #     #         label="Cluster centroids",
        #     #     )

        #     #     ax.set_title(f"{cam.name} – 2D projection, Actual LightGlue Keypoints")
        #     #     ax.axis("off")
...
trans_ess  = np.array([e["trans"] for e in errors_ess])
rot_ess    = np.array([e["rot"] for e in errors_ess])
...
for i in range(3):
    ...
    # axes[i].hist(rot_ess[:,  i], bins=40, alpha=0.6, label="Essential Matrix")
